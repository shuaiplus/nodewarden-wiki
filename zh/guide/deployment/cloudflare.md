...
仓库里的 `database_id` 是占位值，用来让新 clone 的项目在登录前也能通过
Wrangler 配置解析。`npm run deploy` 和 `npm run deploy:kv` 会先运行
`scripts/ensure-d1.cjs`，自动复用或创建 `nodewarden-db`，然后把真实 id 写回
当前使用的 Wrangler 配置文件。
