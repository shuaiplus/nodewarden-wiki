...
NodeWarden's Web Vault is a Progressive Web App with offline capabilities:

**Service Worker:**
- Generated by Vite during build (`webapp/vite.config.ts`)
- Pre-caches app shell (HTML, CSS, JS)
- Runtime caches static resources and API responses
- Network-first strategy with offline fallback

**Offline Storage:**
- **IndexedDB** (`lib/vault-cache.ts`) - Caches vault data (ciphers, folders, sends)
- **localStorage** (`lib/offline-auth.ts`) - Offline unlock records with encrypted profile keys
- **Cache API** - Service Worker managed resource cache

**Web Worker:**
- Background vault decryption (`workers/vault-decrypt.worker.ts`)
- Non-blocking key derivation and cipher decryption
- Preserves UI responsiveness during large vault operations

**Network Detection:**
- Active probing (`lib/network-status.ts`) with 3.5s timeout
- Automatic offline/online state management
- 5-second result caching to avoid excessive requests

**PWA Manifest:**
- Installable app with shortcuts (`public/manifest.webmanifest`)
- Standalone display mode
- Quick actions: Open Vault, TOTP Codes

See [PWA Features](../client/pwa.md) and [Offline Usage](../client/offline-usage.md) for user documentation.
